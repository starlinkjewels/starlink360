...
/*
 * The light tent a diamond is photographed in.
 *
 * A diamond has no colour of its own. Every pixel of it is something else,
 * bent: the refraction material traces rays through the stone and whatever they
 * hit at the end is what you see. It samples ONLY the environment map — scene
 * lights do not reach it at all — so the stone is, quite literally, a picture of
 * the room.
 *
 * Pointing it at `warehouse` therefore renders a warehouse: brown brick, grey
 * concrete, a couple of dim windows. That is the muddy grey-brown a stone
 * should never be, and no amount of IOR, dispersion or fresnel tuning fixes it,
 * because none of those change what the ray lands on.
 *
 * So the stones get their own room. This is the setup a real jeweller shoots
 * in — large white panels above and to the sides, a reflector below, and dark
 * gaps between them. The gaps matter as much as the panels: a facet that sees
 * a gap goes dark, a facet that sees a panel goes white, and that alternation
 * across the crown IS what reads as a diamond. Light it evenly from everywhere
 * and you get a milky glass ball.
 *
 * The metal keeps the warehouse. That is not an inconsistency — it is how the
 * photograph is actually taken, with the piece in a room and the stone in a
 * tent, and it is what makes gold look like gold and diamond look like diamond
 * in the same frame.
 *
 * ── Why this is built by hand rather than rendered ──────────────────────────
 *
 * The obvious approach — build the panels as meshes and bake them with
 * PMREMGenerator — produces a CubeUV atlas, and drei's refraction shader does
 * not read one. Its non-cube branch is `equirectUv(rayDirection)`: it wants a
 * plain equirectangular map, which is what `useEnvironment` hands it. Feed it
 * a PMREM atlas and every ray lands in an arbitrary texel of a packed mip grid,
 * so the stones sample noise and disappear. PMREM is for prefiltering rough
 * reflections anyway; a diamond is perfectly specular and needs none of it.
 *
 * The tent is defined analytically, so the equirect map can simply be
 * evaluated: for each texel, work out the direction it represents and what lies
 * that way. No GPU, no render target, no ordering problem with the render loop,
 * and the result is verifiable outside a browser.
 */

/** A softbox: a flat white panel aimed at the stone. */
interface Panel {
...
/**
 * A panel resolved to a plane and its in-plane axes, oriented the way
 * `Object3D.lookAt(0, 0, 0)` would orient it.
 *
 * Every field is a scalar rather than a vector. The bake tests roughly five
 * million rays, and returning `[x, y, z]` tuples from the vector helpers
 * allocated an array per test — that alone was most of the bake time, which
 * matters when it runs on a phone during load.
 */
interface Frame {
  /** Plane normal, pointing back at the stone. */
  nx: number;
  ny: number;
  nz: number;
  ...
  py: number;
  pz: number;
  /** In-plane axes, pre-divided by the half extents. */
  ux: number;
  uy: number;
  uz: number;
  vx: number;
 ...
 */
export function createGemEnvironment(): THREE.Texture {
 ...
 */
export function getGemEnvironment(): THREE.Texture {
  if (!cached) cached = createGemEnvironment();
  return cached;
}
